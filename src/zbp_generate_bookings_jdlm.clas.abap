CLASS zbp_generate_bookings_jdlm DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zbp_generate_bookings_jdlm IMPLEMENTATION.
...
*   Delete the possible entries in the database table - in case it was already filled
    DELETE FROM ztbooking_jdlm.
*   insert the new table entries
    INSERT ztbooking_jdlm FROM TABLE @it_bookings.

*   check the result
    SELECT * FROM ztbooking_jdlm INTO TABLE @it_bookings.
    out->write( sy-dbcnt ).
